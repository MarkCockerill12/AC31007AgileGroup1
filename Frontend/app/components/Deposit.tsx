...
export function Deposit({ 
  ...
  const handleDeposit = async (amount: number) => {
    ...
    try {
      ...
      if (balanceResponse.RespType === 0) {
        ...
        // Now handle deposit with correct currency conversion
        const convertedAmount = balanceResponse.CrncyType === "1" ? amount / 0.7 : amount;
        
        const response = await handleSendTransaction(2, convertedAmount, CardNumber, PIN, setResponse);
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen">
      ...
      >
        <h2 className="text-black text-3xl mb-6 font-extrabold">{t.deposit}</h2>
        <h3 className="text-black text-xl mb-6">{t.depositAmount}</h3>
          ...
          >
            ...
            <div className="grid grid-cols-3 gap-3">
              {["1", "2", "3", "4", "5", "6", "7", "8", "9", "C", "0", "←"].map((key) => (
                <motion.button
                  key={key}
                  className="p-4 text-xl font-bold text-black bg-gray-100 rounded-lg"
                  onClick={() => {
            ...
            <div className="grid grid-cols-2 gap-3 mt-4">
              ...
              <motion.button
                ...
                onClick={() => {
                  handleDeposit(Number(customAmount));
                  setShowKeypad(false);
                }}
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
